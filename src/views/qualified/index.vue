...
export default {
  ...
  methods: {
    // 刷新验证码
    refreshImg () {
      this.$http.get('http://localhost:8080/captcha').then(res => {
    ...
    // 登录操作
    login () {
      this.$http.post('http://localhost:8080/queryresult', this.loginForm, {
        headers: {
        ...
        .then(res => {
          ...
          this.check()
          if (res.data.isad === '是') {
            alert('您已被录取，录取专业是' + res.data.admaj)
          } else if (res.data.isad === '否') {
            alert(res.data.admaj)
          }
        })
        .catch(error => {
          console.log(error)
